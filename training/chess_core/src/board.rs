//! Pure chess board + AlphaZero encoding (no pyo3 / numpy).
//!
...
//! search). Extracted verbatim from the former `chess_rs/src/lib.rs` so the
//! parity tests keep passing; the pyo3 wrappers now live in the `chess_rs` cdylib.
//!
//! Move handles are a packed u16: from(0..63) | to<<6 | promo<<12, where promo
//! nibble = 0 None else (cozy Piece index + 1). Castling is cozy's "king captures
//! own rook" encoding in the handle (so `push` is native), but its *policy index*
//! uses python-chess's king-two-squares form.

use cozy_chess::{BitBoard, Board as CzBoard, Color, File, Move, Piece, Rank, Square};

const PLANES: i32 = 73; // POLICY_PLANES
const PLANE_AREA: usize = 8 * 8; // per-channel cells
const ENC_CH: usize = 18; // INPUT_CHANNELS
pub const ENC_LEN: usize = ENC_CH * PLANE_AREA; // 1152
...
// piece placement + side + castling + ep *only when legally capturable*). Stored
// exactly (no hashing) so there are zero false repetitions.
...
impl Board {
    ...
    pub fn from_fen(fen: &str) -> Result<Self, String> {
        let inner = CzBoard::from_fen(fen, false).map_err(|e| format!("bad fen: {e:?}"))?;
        let b = Board {
            inner,
            history: Some(Vec::new()),
        };
    ...
    pub fn push(&mut self, handle: u16) {
    ...
    pub fn move_to_index(&self, m: Move) -> i32 {
    ...
    pub fn outcome_white(&self) -> Option<i8> {
        match self.outcome() {
            None => None,
            Some((false, _)) => Some(0),
